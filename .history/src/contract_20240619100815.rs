...
mod tests {
    ...
    fn test_execute_vote_valid() {
        ...
        assert!(res.is_ok(), "Instantiation failed: {:?}", res.err());
        ...
        assert!(res.is_ok(), "Create poll failed: {:?}", res.err());
        ...
        let res = execute(deps.as_mut(), env.clone(), info.clone(), msg);
        assert!(res.is_ok(), "First vote failed: {:?}", res.err());
        ...
        let res = execute(deps.as_mut(), env.clone(), info.clone(), msg);
        assert!(res.is_ok(), "Second vote failed: {:?}", res.err());
    }
    ...
    fn test_execute_vote_invalid() {
        ...
        // Vote on a non-existing poll (should fail)
        let msg = ExecuteMsg::Vote {
            poll_id: "some_id".to_string(),
            vote: "Juno".to_string(),
        };
        let err = execute(deps.as_mut(), env.clone(), info.clone(), msg);
        assert!(err.is_err(), "Expected error when voting on a non-existing poll");
        ...
        // Vote with an invalid option (should fail)
        let msg = ExecuteMsg::Vote {
            poll_id: "some_id".to_string(),
            vote: "DVPN".to_string(),
        };
        let err = execute(deps.as_mut(), env, info, msg);
        assert!(err.is_err(), "Expected error when voting with an invalid option");
    }
